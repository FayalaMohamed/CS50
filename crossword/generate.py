    ...
    def consistent(self, assignment):
        ...
        for var in assignment.keys():
            if var.length != len(assignment[var]):
                return False
            if (assignment[var] in distinctValues):
                return False
            else:
                distinctValues.append(assignment[var])

            """ for var2 in assignment.keys():
                if assignment[var] == assignment[var2]:
                    return False """

            neighbourCells = self.crossword.neighbors(var)
            for neighbor in neighbourCells:
                overlap = self.crossword.overlaps[var, neighbor]
                if (
                    neighbor in assignment
                    and assignment[neighbor][overlap[1]] != assignment[var][overlap[0]]
                ):
                    return False
